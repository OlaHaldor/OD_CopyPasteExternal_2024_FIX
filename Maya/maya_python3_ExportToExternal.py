...
# Using the temporary directory for the file
...
def fn_getObjectData(exportedObj):
    ...
    if len(curSel) > 0:
        ...
        for v in range(0, exportedObj.vertexCount):
        ...
        for f in range(0, exportedObj.polyCount):
...
def fn_saveTempObject(exportedObj, exportFilename):
...
def main():
    ...
    objectOK = fn_getObjectData(exportedObj)
    if objectOK:
        fn_saveTempObject(exportedObj, exportFilename)
